#import "template.typ": *
#show: ieee.with(
  title: [Crack on the RSA Encryption],
  abstract: [
    Although RSA is proven to be exceedingly secure theoretically, for which it has attained a popularity all over the world, it is practically crackable in accpetable cost of time. The improper generation of RSA key pair might lead to the vulnerability. In this report, we introduce six approaches to attack the RSA encryption algorithm in the specific situation, which is generally categroised into three parts according to the aspect of weakness. Meanwhile, we present the theoretical proof and the demo code to verify the feasibility of the approaches.
  ],
  authors: (
    (
      name: "Ben Chen",
      department: [Student],
      organization: [Department of Computer Science and Engineering, SUSTech],
      location: [Shenzhen, China],
      email: "chenb2022@mail.sustech.edu.com"
    ),
  ),
  index-terms: ("RSA", "Cryptography", "Cybersecurity"),
  bibliography-file: "refs.bib",
)

= Introduction
The security of modern cryptography relies on the difficulties on solving the _NP_ problems within a acceptable cost, such as the factorization of large numbers. Generally, modern cryptography is categorised into three majar parts, Symmtric (AES-ECB), Asymmetric (RSA, ECC) and Homomorphic (Lattice). However, the cryptography system is never absolutely secure and brilliant mathematicians and hackers have proposed several methods to crash into the wall built by cryptography. With the vulnerability found, we are able to crack the cryptography with polynomial cost of time, which turns the _NP_ problem into _P_ problem.

In this report, we mainly focus on the RSA algorithm which is one of the most well-known and widely applied cryptography algorithm nowadays. The following sections will give a brief introdution to the RSA and then we will move to the most exciting part, the techniques to hack the cryptography where we also provides the python codes as proof of concept.

= Background
The RSA algorithm is a typical asymmetric cryptography algorithm, which is known for the public key and private key. The reliability has its base on the hardness of factoring large numbers which are usually in length of 1024 bits or more. Therefore, the brute force algorithm is unlikely to achieve an accpetable solution, unless you have a quantum computer. Nevertheless, crack the encryption rudely is not cool for a hacker or computer science lover. In this section, we just simply introduce how RSA works and attack methods will not be included.
...
Then the public key will be $(N,e)$ and the private key is $(N,d)$. Note that in the next section the first step might be different, largely to improve the reliability.
...
so $f_0 = 1, f_(k+1) = 2f_k + b_(k+1)$ then $f_t = r$, as $V_0 (P) = 2$, $V_1 (P) = P$, then the formular is
...
where $t = 1, c+ 1, 2c+1, dots, c[B_2 slash c] + 1$.

When $(Delta slash p) = -1$ we have
$ p divides H_i $
so end of proof.
...
Hence
$ gcd(e x - z, N) \ = gcd(p^(r-1)(p-1)(q-1) y, p^r q) = g $
...
$ abs(x - a/b) < 1/(2 b^2) $
$a slash b$ is a continued fraction approximation of $x$. So when
$ d < sqrt(2) / (2g) N^(1/4) $
we have $k slash (d g)$ is a continued fraction approximation of $e slash N$, and we can cover all the possible private keys $d$ by expanding the coninued fractions.
