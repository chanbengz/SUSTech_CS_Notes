 ...
 * @width:      maximum number of digits, if the actual width is less than
 * @width, use @padc instead
 * @padc:       character that padded on the left if the actual width is less
 * than @width
 * */
...
/* *
 * getint - same as getuint but signed, we can't use getuint because of sign
 * extension
 * @ap:         a varargs list pointer
 * @lflag:      determines the size of the vararg that @ap points to
 * */
...
/* *
 * vprintfmt - format a string and print it by using putch, it's called with a
...
/* *
 * vsnprintf - format a string and place it in a buffer, it's called with a
 * va_list instead of a variable number of arguments
 * @str:        the buffer to place the result into
 * @size:       the size of buffer, including the trailing null space
 * @fmt:        the format string to use
