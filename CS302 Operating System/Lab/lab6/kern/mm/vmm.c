...
static void check_pgfault(void)
{
	...
	nr_free_pages_store--; // szx : 适应三级页表
 ...
 * information to aid in diagnosing the exception and recovering from it. (1)
 * The contents of the CR2 register. The processor loads the CR2 register with
 * the 32-bit linear address that generated the exception. The do_pgfault fun
 * can use this address to locate the corresponding page directory and
 * page-table entries. (2) An error code on the kernel stack. The error code for
 * a page fault has a format different from that for other exceptions. The error
 * code tells the exception handler three things:
 *         -- The P flag   (bit 0) indicates whether the exception was due to a
 * not-present page (0) or to either an access rights violation or the use of a
 * reserved bit (1).
...
int do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr)
{
   ...
   * for la if the PT contians this pte didn't exist, alloc a page for PT
   * (notice the 3th parameter '1') pgdir_alloc_page : call alloc_page &
   * page_insert functions to allocate a page size memory & setup an addr map
   * pa<--->la with linear address la and the PDT pgdir DEFINES: VM_WRITE  : If
   * vma->vm_flags & VM_WRITE == 1/0, then the vma is writable/non writable
   *   PTE_W           0x002                   // page table/directory entry
   * flags bit : Writeable PTE_U           0x004                   // page
   ...
   */
	...
	if (*ptep ==
	    0) { // if the phy addr isn't exist, then alloc a page & map the
